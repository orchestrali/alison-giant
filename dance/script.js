...
$(function() {
  ...
  for (let p of q) {
    if (p[0] === "colorscheme") {
      $("#colorscheme").val(p[1]).change();
    }
  }
  
  
  $.get("dance/music.json", function(data) {
    music = data;
  ...
  $('#dance').change(function() {
    ...
    j = 0;
		...
		if (value) {
			...
			$('#wait').show();
			$.get("dance/dances/"+value, setDance);
		}
	});
  loadList();
  getTypes();
...
function loadList() {
	$.get('dance/dances/a-dance-list.xml', function(data) {
			var allTypes = [];
			var options = '';
...
function getTypes() {
  $.get("dance/dancetypes.json", function(data) {
    dancetypes = data;
...
function dancercolor(d) {
  if ($("#colorscheme").val() === "lark") {
    ...
    let color = ["#fc3434", "#ca85ff", "#46b126", "#fcd634"][num];
    return {fill: color, stroke: "black"};
    //["#fc3434", "#b654ff", "#46b126", "#fcd634"][num] : ["#ff7171", "#ca85ff", "#7fd166", "#fee477"][num]
  }
...
function drawInitial(svg) {
  ...
  svg.rect(innerlark, -50, -5, 100, 10, 5, 5, {fill: "black"});
...
// run a set of animations at a given time
function animate(a, j1, i, jump) {
  ...
  a.forEach(obj => {
    if (obj.start <= repeats || !obj.start) {
